...
import com.xingxunlei.bean.MailAuthenticator;
...
public class MailSendUtil {
    ...
    public static boolean sendMail(MailSenderInfo senderInfo) {
        ...
        if ("true".equals(senderInfo.getValidate())) {
            // 如果需要身份认证，则创建一个密码验证器
            authenticator = new MailAuthenticator(senderInfo.getUserName(), senderInfo.getPassword());
        }
        ...
        try {
            ...
            String nick = "";
            try {
                nick = MimeUtility.encodeText(senderInfo.getFromNick());
            } catch (UnsupportedEncodingException e) {
                nick = "";
            }

            // 设置邮件消息的发送者
            sendMailMessage.setFrom(new InternetAddress(nick + " <" + from + ">"));
